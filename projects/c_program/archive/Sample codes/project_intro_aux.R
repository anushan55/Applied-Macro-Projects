...
A <- paste0(HOME,"/project_anushan/a_microdata/") #here: path to folder with data
TEMP <- paste0(A,"/","temp",sep="") # path to subfolder with intermediate output, optionally reset/ deleted/created anew

D <- paste0(HOME,"/project_anushan/d_results/") #here: path to folder with output (tables, figures, logs)
GEO <- D
FIGURE <- D
LATEX <- D
LOG <- D
...
DFBASE <- "project_databasics"
DFMERGE <- "project_varcreation_merge"
DFMAIN <- "project_samplecreation_main"
