...
A <- paste0(HOME,"/project_mthesis/a_microdata/") #here: path to folder with data
TEMP <- paste0(A,"/","temp",sep="") # path to subfolder with intermediate output, optionally reset/ deleted/created anew

D <- paste0(HOME,"/project_mthesis/d_results/") #here: path to folder with output (tables, figures, logs)
GEO <- D
FIGURE <- D
LATEX <- D
LOG <- D
...
PROJECTNAME <- 'project_mthesis'
PROJECT <- paste0(HOME,PROJECTNAME,"/") #direct path to directory of specific project
SEED <- 1234567
