...
A <- paste0(HOME,"/project_m6practice/a_microdata/") #here: path to folder with data
TEMP <- paste0(A,"/","temp",sep="") # path to subfolder with intermediate output, optionally reset/ deleted/created anew

D <- paste0(HOME,"/project_m6practice/d_results/") #here: path to folder with output (tables, figures, logs)
GEO <- D
FIGURE <- D
LATEX <- D
LOG <- D
...
DFBASE <- "m6practice_databasics"
DFMERGE <- "m6practice_varcreation_merge"
DFMAIN <- "m6practice_samplecreation_main"
...
PROJECTNAME <- 'project_m6practice'
PROJECT <- paste0(HOME,"/project_m6practice/",PROJECTNAME,"/") #direct path to directory of specific project
SEED <- 1234567
...
sink(paste0(D,"/",MAINNAME,"_","sessionInfo"), split = TRUE) #log on
sessionInfo()
sink()
